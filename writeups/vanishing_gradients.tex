...
First consider positive $W$. This means $W$ admits an orthogonal decomposition, $W = CVC^T$ and $||W|| = ||V|| = \lambda_{max}(V)$.
...
then: $||W||\delta_i \leq 1$ and $|W||\delta_s \geq 1$. For this we will assume that $\delta_i > 0$ as otherwise this would that the hidden states vanish, which is
...
So consider: $h^{(t)} = W^th^{(0)}$, meaning we now have $J_t = W^t$. And by the orthogonal decomposition:
\begin{align*}
    W^t &= (CVC^T)^t \\
    &= CV^tC^T
\end{align*}
Thus the behaviour of gradient is mostly captured by $V^t$ as $C$ is unitary. But, as $t$ increases, all entries
of $V$ that are not equal to 1, either vanish or explode. Thus as $t$ increases $V$ quickly becomes singular, and the largest eigenvalue of $V$
will dominate gradient behaviour, which will diminish the expressive power of the network over longer sequences. The only case where $V$ remains stable
